...
/**
 * 外观设置（11.4a）：深浅模式 + Material You 动态取色。读写都走 [SettingsPreferences]（DataStore）。
...
class AppearanceSettingsViewModel @Inject constructor(
    ...
    /** 主题配色家族（暖陶/青花·与深浅正交）。默认暖陶，用户切换即时落 DataStore、根部主题即时换。 */
    val palette: StateFlow<ThemePalette> = settings.themePalette
    ...
    fun setPalette(palette: ThemePalette) {
        viewModelScope.launch { settings.setThemePalette(palette) }
    }
